...
function drawGradientRect(_x, _y, _w, _h, _mainHue, _mainSat, _mainBri, _seed, _targetGraphic, _m4 = 0.5) {
    randomSeed(_seed);
    noiseSeed(_seed);
    ...
    let hueB = processHue(_mainHue + editRandom(30, 60));
    let satB = _mainSat + editRandom(-10, 10);
    let briB = _mainBri + editRandom(-10, 10);

    let isLeftRightGradient = editRandom() < 0.5;
    let isVertical = editRandom() < 0.5;

    let colorDataA = new ColorData(hueA, satA, briA);
    let colorDataB = new ColorData(hueB, satB, briB);

    // m4 controls line count (inverse of spacing)
    let lineCountFactor = 0.2 + _m4 * 1.0; // 0.2 to 1.2
    let xSpacing = _w / (editRandom(10, 60) * lineCountFactor);
    let ySpacing = _h / (editRandom(10, 60) * lineCountFactor);
    ...
    for (let x = 0; x < _w; x += xSpacing) {
        for (let y = 0; y < _h; y += ySpacing) {
            ...
            nowColor.h += editRandom(-6, 6);
            nowColor.s += editRandom(-6, 6);
            nowColor.b += editRandom(-6, 6);

            _tempCardGraphic.stroke(nowColor.h, nowColor.s, nowColor.b);
            _tempCardGraphic.strokeWeight(editRandom(2, 4));
...
function drawStyledRect(_x, _y, _w, _h, _mainHue, _mainSat, _mainBri, _seed, _targetGraphic) {
    ...
    let xSlices = int(editRandom(10, 60));
    let ySlices = int(editRandom(10, 60));

    let sliceRandom = editRandom();
    if (sliceRandom < 0.3)
        xSlices = 1;
    ...
    for (let x = 0; x < xSlices; x++) {
        for (let y = 0; y < ySlices; y++) {
            ...
            let drawHue = processHue(_mainHue + editRandom(-10, 10));

            let drawSat = _mainSat + editRandom(-10, 10);
            let drawBri = _mainBri + editRandom(-10, 10);
